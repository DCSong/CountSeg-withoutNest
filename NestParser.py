            ...
            if is_variable(val):
                ...
                for sub_idx, sub_val in enumerate(val):
                    if is_variable(sub_val):
                        ...
                        val[sub_idx] = parse_config(
                            sub_val, env_vars=env_vars, global_vars=global_vars)
            elif isinstance(val, dict):
        ...
        if param_file is not None:
            ...
            for idx, param in enumerate(param_list):
                ...
                if isinstance(param, dict):
                    ...
                    raise TypeError(
                        'Parameter file should define a list of Dict[str, Any]. Got "%s" in it.' % param)
                # record parameters
                ...
                resolved_config = parse_config(
                    deepcopy(config), env_vars=env_vars, global_vars=global_vars)
                check_all_resolved(resolved_config)
                if verbose:
                    end_time = datetime.now()
                    logger.info('Finished (%s).' % (U.format_elapse(
                        seconds=(end_time - param_start_time).total_seconds())))
        else:
            resolved_config = parse_config(config, env_vars=env_vars)
            check_all_resolved(resolved_config)

        end_time = datetime.now()
        logger.info('All finished. (%s)' % U.format_elapse(
